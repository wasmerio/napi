...
#include "internal/napi_handle_scope_wrapper__.h"
...
#include "internal/napi_setter_callback_info__.h"
...
napi_status NAPI_CDECL napi_add_env_cleanup_hook(node_api_basic_env env,
  ...
  auto* entry = new (std::nothrow) napi_env_cleanup_hook__(napiEnv);
...
napi_status NAPI_CDECL napi_create_buffer(napi_env env,
  ...
  *data = backing->Data();
  ...
  auto* record = new (std::nothrow) napi_buffer_record__(env);
