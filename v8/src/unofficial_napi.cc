...
static napi_status GetLeaseByteView(napi_env env,
                                    napi_value value,
                                    uint8_t** data,
                                    size_t* length) {
  bool matches = false;
  ...
  if (matches) {
    ...
    if (element_length > SIZE_MAX / element_size) return napi_invalid_arg;
  ...
  if (status != napi_ok || !matches) return status == napi_ok ? napi_invalid_arg : status;
...
napi_status NAPI_CDECL unofficial_napi_acquire_buffer_lease(
  ...
  if (env == nullptr || value == nullptr || lease_out == nullptr || data_out == nullptr ||
      mode < unofficial_napi_buffer_access_read ||
  ...
  if (status != napi_ok) return status;
  if (byte_offset > length || byte_length > length - byte_offset) return napi_invalid_arg;

  auto* lease = new (std::nothrow) unofficial_napi_buffer_lease__;
  if (lease == nullptr) return napi_generic_failure;
...
napi_status NAPI_CDECL unofficial_napi_release_buffer_lease(
  ...
  (void)modified;
  if (env == nullptr || lease == nullptr) return napi_invalid_arg;
