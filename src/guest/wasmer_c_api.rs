...
pub(crate) const WASM_C_API_MODULE_NAME: &str = "wasm_c_api_v0";
const WASM_C_API_MODULE_PREFIX: &str = "wasm_c_api_v";

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum WasmCAPIVersion {
    V0,
    Unknown,
}
...
impl WasmCAPIVersion {
    pub(crate) const fn is_compatible_with(self, other: Self) -> bool {
        matches!((self, other), (Self::V0, Self::V0))
    }
}

pub(crate) fn module_needs_wasm_c_api(module: &Module) -> Option<WasmCAPIVersion> {
...
pub(crate) struct WasmCapiSession {
...
impl WasmCapiSession {
    ...
    pub(crate) fn needs_imports(&self) -> bool {
    ...
    pub(crate) fn set_memory(&self, store: &mut StoreMut<'_>, memory: &Memory) -> Result<()> {
    ...
    pub(crate) fn set_table(&self, store: &mut StoreMut<'_>, table: &Table) -> Result<()> {
        let Some(func_env) = self.func_env()? else {
            return Ok(());
        };
...
pub(crate) struct WasmCapiRuntimeHooks {
...
#[derive(Default)]
pub(crate) struct WasmCapiState {
    next_handle: u32,
